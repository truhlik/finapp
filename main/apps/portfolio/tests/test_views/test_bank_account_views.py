...
from ...models import BankAccount, Portfolio


class BankAccountViewSetUnauthorizedTestCase(BaseTestCase):
    view_basename = 'bank-account'
    model_class = BankAccount
    recipe_name = 'bank_account'
    ...
    def test_list(self):
        ...
        baker.make_recipe('main.apps.portfolio.portfolio', category=Category.get_bond(), active=True, owner=self.get_user())
...
class BankAccountViewSetAuthorizedTestCase(BaseTestCase):
    view_basename = 'bank-account'
    model_class = BankAccount
    recipe_name = 'bank_account'
    ...
    def test_list(self):
        ...
        baker.make_recipe('main.apps.portfolio.{}'.format(self.recipe_name), category=Category.get_bond(), active=True, owner=self.get_user())
        baker.make(self.get_model_class(), category=self.get_category_instance(), active=True)
        baker.make(self.get_model_class(), category=self.get_category_instance(), active=False, owner=self.get_user())
        self.client.force_authenticate(self.get_user())
        r = self.client.get(reverse('{}-list'.format(self.view_basename)))
        self.assertEqual(200, r.status_code)
